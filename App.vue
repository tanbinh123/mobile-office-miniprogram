...
export default {
	onLaunch: function() {
		...
		//先拉取网站信息
		this.tui.request("/", "GET", {}, false,true,true).then((res) => {
			res = res.data;	
			console.log(res)
			uni.setStorageSync('office_state',res.stateItems);
			uni.setStorageSync('office_department',res.orgs);
			uni.setStorageSync('office_post',res.posts);
			uni.setStorageSync('office_version',res.version);
			uni.setStorageSync('office_version_msg',res.versionMsg);
		}).catch((res) => {
			this.tui.toast("服务器连接失败", 2000, true);
			console.log(res);
		})
