...
const tui = {
	...
	request: async function(url, method, postData, isDelay, isForm, hideLoading) {
		...
		return new Promise((resolve, reject) => {
			//看是否拼接
			if(url.substr(0,1) == "/") url = tui.interfaceUrl() + url;
			uni.request({
				url: url,
				data: postData,
				header: {
	...
	setUserInfo: function(mobile, token) {
		//uni.setStorageSync("office_token", token)
		uni.setStorageSync("thorui_mobile", mobile)
	},
	//获取token
	getToken() {
		return uni.getStorageSync("office_token")
	},
	//判断是否登录
	isLogin: function() {
		return uni.getStorageSync("office_token") ? true : false
	},
	//跳转页面，校验登录状态
	href(url, isVerify) {
